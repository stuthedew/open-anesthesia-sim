...
**The fixture-id guard is no longer here.** `test_every_fixture_id_is_one_the_store_could_mint`
globbed this directory and only this directory, so the apparatus fixtures one
level out were held to no id grammar at all and `tools/generator_check.py` could
print `PL-AAAA` as the line for a session to copy. It is `tools/fixture_id_check.py`
now, which reads `store.ID_RE` exactly as the test did and scans every Python
file in the repository plus every document under `.claude/` (`PL-7922`). Moved
rather than duplicated: two spellings of one grammar drift, which is why
`store.py` exports `ID_PATTERN` unanchored instead of letting each reader write
its own. The cost is that this subproject, extracted on its own, would carry no
fixture-id guard until it took a copy.
"""
