...
of their own (`#744` lands 464 characters against a 3,399-character body).
...
of 654 still match with only the line breaks inside a paragraph joined, so
whitespace is compared as a single space. The 2026-09-06 signing rewrite
...
of line breaks, or only of one commit hash replaced by another.
...
#: Anchored to the end, so a trailer quoted inside the body is compared.
...
#: are never masked: a hash spelled in one class alone then shows up as a
#: difference, which a reader can dismiss, rather than a number that changed
#: passing as the same.
