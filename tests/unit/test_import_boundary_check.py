...
def _allowed_modules(package: Path) -> Path:
    ...
    makes the import, is itself an error - which is what keeps a stale
...
class TestAnalyze:
    ...
    @pytest.mark.parametrize(
        "package",
        ["time", "datetime", "random", "secrets", "uuid", "PySide6", "pyqtgraph", "numpy"],
    )
...
class TestMain:
    ...
    def test_the_toolkit_and_its_numerics_are_refused_under_core(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ) -> None:
        """The port's three packages, refused in a compartment before any of them exists.

        Written against the real `BOUNDARIES` and a tree in which no module
        under `src/` imports any of the three yet, which is the state the
