...
# is (`PL-TRMN`). A quoted argument stays one word, so a `;` or a flag inside
# it is the text it is, where the regex this file used took a `;` inside
# quotes for a separator (`PL-WGFY`). Heredoc bodies and comments are gone,
# which is what lets this repository write prose *about* pruning -
# `CLAUDE.md`, this hook, the ledger - without the guard eating its own
# documentation; a prune after the terminator of a heredoc is read like any
# other line (`PL-39LD`).
#
