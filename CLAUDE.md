...
- **Answer "what should we work on next?" from the file.** Lead with the
  ...
  codebase. Recommending an item and starting it in the current session
  are different acts: when this session is already long or was about
  something else, hand the item to a fresh one with the entry and the
  notes it cites, per the session-efficiency rules above. `P0` items come first and are handled as hotfixes: their own
