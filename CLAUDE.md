...
on 09-20 and 09-21, `PL-KVDK`). **It holds apparatus work unrelated to fixing
the generators: new features and housekeeping** (project owner, 2026-09-23). A
new command, check, field or rule of that kind, or a re-specification of the
generator rule, is captured and not built, nor offered as the answer to a
finding. Work on the generators is what the pause is for, and needs no lift:
fixing a live generator, a defect in what exists, or the machinery that finds
and ranks generators, including what a head records (`PL-5MYR`). The pause ends when no open item carries `generator: live` — no
...
**A decision the owner *ratified* and one they *specified* are not the same
decision, and they have different bars to reopen** (project owner, 2026-09-16).
"Agree with recs" means the case looked sound on a read that may not have caught
every nuance - not that the question is closed. A user-facing behavior they
asked for in their own words is firmer. Four consequences:
...
- **Reopen a ratified decision on ordinary evidence**: a measurement, a cost the
  ...
  was a session's own recommendation wearing their signature.
- **Reopen a specified one only on a compelling argument**, and say plainly that
  is what you are doing.
- **The safety-critical standard overrides both.** A wrong or misleading
  clinical value is not a preference either kind of decision can settle.
...
- **Housekeeping you are about to do yourself is filed before you do it.** The
  ...
  is filed, claimed or committed and no pull request opens (project owner,
  2026-09-26, ratified, over filing branch cleanup as housekeeping with a pull
  request of its own, `PL-S8LZ`). `.github/workflows/branch-sweep.yml` deletes
  finished `claude/*` branches daily, so the reply runs
  `python3 tools/branch_sweep.py` and hands the owner only a branch it keeps
  that should go anyway (project owner, 2026-09-26, ratified, over a list
  built by hand, `PL-X8SV`).
- **A root cause of more than two items is pulled, not queued** (project owner,
  ...
  the claim is a recorded fact rather than the next session's inference.
  **The count decides the record; expected recurrence decides the rank**
  (project owner, 2026-09-21, ratified, over keeping the count for both and
  accepting that any three-item cluster outranks a `safety`-classed `P1`). A
  generator the store is still handing new members ranks **above everything but
...
- **Commit and push as you go; the pull request arrives with the work**
  ...
  withdrawal and asking per pull request while it is off). Then
...
- **Bring `origin/main` into an open pull request once, when it is armed or
  ...
  2026-09-23, ratified, over a merge queue, which a user-owned repository
  cannot have), and each base merge re-runs `quality.yml`, so one taken while
...
**Two standards, deliberately unequal.** The simulator and the documentation a reader of it needs — `src/`, `tests/`, `docs/MODEL.md`, `README.md` — are held to the specialist standard named in the paragraph above: code an expert contributor would recognize as high quality and could maintain without explanation. The workflow apparatus that exists so agent sessions can be productive — `subprojects/docket/`, `tools/`, `.claude/`, `.github/`, `docs/worker.md`, and this file — is held to a lower and different bar, which is `.claude/rules/apparatus-standard.md` and loads only when a session opens one of those paths. **Nothing in it reaches the simulator**, and it is kept out of this file so that it cannot: a session applied it to `src/` while it was resident here, correctly quoting a sentence whose scope was three sentences away (`PL-6SBB`). Where the two compete for a session, the simulator wins.
