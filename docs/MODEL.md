...
difference the interface permits, because the percent axis is fixed at 3 MAC
for every agent (`CHART_AXIS_TOP_MAC`) and the slow compartments sit near zero
on it - fat within about 1% of its height through a case's first hour. Those
are the compartments this chart exists to teach. It is consequential rather than
