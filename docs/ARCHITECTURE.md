...
├── pr_body_check.py      # records a pull request's body as `docs/pr-bodies/<N>.md` on its branch before the merge, and holds the merge on it, because that file rather than the squash commit's body is the permanent record of a change's reasoning: the squash body on `main` is a derived copy, useful in `git log` and nothing more, so its hard-wrap, its freeze when auto-merge is armed and any client's rewrite stop being defects in the record (`PL-979D`); `--record [N]`, run once the pull request is open and again after any edit to its body, fetches the body from the API rather than writing what the session sent, since GitHub's copy is what a reader and the merge meet and the server rewrites bodies silently (`PL-1DN9`), and for a pull request already merged writes a recovered file whose header dates the fetch and says how the squash copy compares; `--check`, a step of `pr-title`'s required job that runs on `edited` as well as on pushes, fails unless the body is empty or the head's record holds the same body, so a body edited after it was recorded holds the merge until it is recorded again; `--anchors`, in `make check`, refuses a recovered file whose `commit:` is not a first-parent commit of the default branch, and says it checked nothing on a shallow clone; the default mode stays the one-line session-start advisory for a squash commit that landed with an empty body and no record, which since `--check` can name only a pre-rule merge or an admin bypass of it, with `--recover` fetching that body back - forward only, so the pre-rule squash commits are not backfilled, and `--compare` is a hand-run measurement of how far their bodies drift from their pull requests
...
gates, and `pr_body_check.py`'s `--record`, `--recover` and `--compare` modes do. (This said it was the only
