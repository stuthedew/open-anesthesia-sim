...
`app/chart_series.py` builds the traces and redraws them from the states
`drawn_window` evaluates, `app/control_timeline.py` turns the recorded
control changes into
...
columns). `app/chart_series.py` is the layer above it — it holds the
