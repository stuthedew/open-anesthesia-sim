...
├── main_ci_status.py     # reports the default branch's last quality verdict, and nothing at all when it was a success, because the whole-store `verify:` replay runs only on push to `main` and its failures therefore land on a run no pull request shows; on a failure it makes a second request for the run's jobs and names the step that failed, because 93 of the 109 failures on record are the whole-store replay alone and a line naming no step cannot separate those from a broken tree (`PL-T83R`); the session-start hook calls it, it gates nothing, and it is not the only tool here that reads the network - `required_checks_check.py` does too and gates, and `pr_body_check.py` does in its `--recover` mode
├── open_pull_requests.py  # prints the branch name of every open pull request, one per line, so `bin/docket flight` can tell a branch waiting on review from one nobody opened - the half of "is anybody still on this branch" that git cannot answer, and the half that left `claude/hopeful-allen-tetrje` reported as live work for three hours with every item on it closed; the contract is the exit status, zero meaning it looked and non-zero with an empty stdout meaning it could not, because "could not look" read as "nothing is open" would announce that finished work has stalled on every branch under review; it lives here rather than in `subprojects/docket/` for the reason `main_ci_status.py` does - that package answers from a bare checkout with no network and knows nothing about GitHub - and `docket.toml`'s `open_pull_requests_command` is what points `flight` at it; `pr_title_check.py` shares the request rather than making its own, so the token, the timeout and the every-failure-is-a-skip rule have one spelling
├── possessive_section_check.py  # reports a possessive citation whose quotation matches a heading or `**Bold.**` marker in the file it names, and prints the `§` line it should become, because admitting the possessive to `CITATION_CONNECTIVE` (`PL-316G`) settled whether such a citation is *checked* and left what it tells a reader: `§` says the quotation is a section title and the possessive says nothing, this project writing it to quote a sentence as often as to cite a section. Only the direction that is a fact about the tree is reported - a quotation matching a heading is a section citation - while one matching no heading may be a faithful quotation of prose and is left alone, which is the judgment half `CLAUDE.md` refuses to script. Closed briefs never reach it, `_quoting_sources` having exempted them since `PL-ZM8P`
├── pr_body_check.py      # reports a squash commit on the default branch that landed with an empty message body, because this repository's squash commit message *is* the pull request body (`squash_merge_commit_message: PR_BODY`) and 187 of 683 squash commits on `main` arrived without one - 763,224 characters of design reasoning that survive only on GitHub, where `git log`, `git blame` and every checkout cannot reach them; the predicate is exact, a first-parent subject ending in `(#N)` with nothing under it and no `docs/pr-bodies/<N>.md` recording it, which is what keeps it off the twenty commits that legitimately carry no body - the 2026-08 bootstrap commits and the old `Merge pull request #N from ...` merges, neither shape ending in `(#N)`; advisory rather than a gate, since a merge creates the condition and no branch running a check ever caused it, and it runs from `.claude/hooks/docket-digest.sh` beside `main_ci_status.py` rather than from `make check` because the remedy commits files outside the current item's `touches`, which the fix-now rule forbids inline - so in `make check` it would fire at the one moment a session may not act on it, while session start is before an item is picked and is also the only place `origin/main` is freshly fetched; one line, because session-start output is resent every turn, and `--recover` is a separate mode because repair reads the network while detection must stay green offline; a pull request that genuinely has no body is tombstoned rather than reported forever, `#325` being the live case; the mechanism is an explicitly empty `commit_message` on the merge request, which GitHub honours literally over the repository default - 9 of 9 affected pull requests whose `auto_merge` object survived carry `commit_message: ""` against 0 of 73 unaffected - while what sends it is a per-sitting property rather than anything about the pull request, merges under 60 seconds apart agreeing 98.3% of the time against 60.2% by chance and owner-local hour separating at permutation p = 5e-5; GitHub records no client identity, so prevention needs a change of merge client and is the owner's rather than the tree's, which is why this detects and repairs instead
...
gates, and `pr_body_check.py`'s `--recover` mode does. (This said it was the only
...
The five scripts `.claude/settings.json` wires as hooks live in
...
`docket-branch-guard.sh`, `no-prune-guard.sh` and `floor-interpreter-guard.sh`
are documented where their behavior is, in their own headers.
