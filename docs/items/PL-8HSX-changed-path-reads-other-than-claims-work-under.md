...
classes: defect
feature: one-answer
touches: subprojects/docket/src/docket/vcs.py, subprojects/docket/src/docket/verify.py, subprojects/docket/src/docket/claims.py, subprojects/docket/tests/test_verify.py, subprojects/docket/tests/test_vcs.py, subprojects/docket/tests/test_git_runner.py, tools/doc_check.py
...
- **A path whose bytes are not UTF-8 now reads as unread instead of raising.** Printed as written, a Latin-1 `café.py` is the byte `\351`, and `subprocess.run(text=True)` raised `UnicodeDecodeError` out of `vcs._run_git`, whose contract is that nothing raises (measured on Linux; quoted, it had been `"caf\351.py"`). `_run_git` answers it as a silence, and `verify._git` and `doc_check._git` as `GitUnanswered`, so the audit stops rather than passing.
