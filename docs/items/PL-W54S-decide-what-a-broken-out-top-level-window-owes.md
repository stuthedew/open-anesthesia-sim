...
**Done when.** `docs/MODEL.md` § "Minimum displayed outputs" states what the unconditional region owes in a top-level window other than the one carrying it - replicated per window, or break-out restricted to editors that carry their own context, or another answer with its reason recorded - the hazard table's "always visible" row is qualified to match, and the occlusion paragraph distinguishes a second window that covers the region from one read alone.
