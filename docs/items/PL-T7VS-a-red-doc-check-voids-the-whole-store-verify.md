...
**Re-pointed by `PL-6TP8`, 2026-09-19.** The contract records what this brief
found and names it: a prerequisite clause makes a non-zero exit unreadable, so
the replay reads *nothing* from a plain failure - the "void" is an under-report
in the replay's one direction, never a wrong finding, and the headline was true
rather than lucky. Measured the same day with every prerequisite clause
stripped: no discriminator among the 162 such commands exits 0, so nothing is
masked today either. What happens to this item is the shape half of
`PL-6TP8`'s decision. If the field drops the prerequisite clause, this item
