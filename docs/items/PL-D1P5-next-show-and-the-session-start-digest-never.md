...
**Closed 2026-09-26 with `PL-XBV4`.** `next`, `show` and the digest fetch
once through the snapshot - one call per command, however many reads beneath
it ask - and, told not to, date the refs they read with the clock time and the
age. The hook runs `digest --no-fetch` after `branch --brief`'s fetch, so the
digest now says what it rests on instead of being fresh by the hook's
ordering. `test_next_show_and_digest_do_not_read_unfetched_refs_as_fresh`
holds the three.
