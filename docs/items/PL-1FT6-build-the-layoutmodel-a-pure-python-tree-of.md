---
id: PL-1FT6
title: Build the LayoutModel: a pure-Python tree of Splits and Areas with split, join, resize, swap and set_editor, a borders() collinear-chain query, and versioned JSON serialization over a root that holds one or more windows from v1
status: ready
...
**Problem.** Build the LayoutModel: a pure-Python tree of Splits and Areas with split, join, resize, swap and set_editor, a borders() collinear-chain query, and versioned JSON serialization over a root that holds one or more windows from v1
...
Qt import, exercising split, join, resize, swap, `set_editor`, `borders()` and a
