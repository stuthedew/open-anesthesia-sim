...
**Problem.** Tag v0.5.11 on the merge commit of PL-DRRG's cut: the release is cut and only the project owner can push a tag ref from this environment

**`PL-DRRG` cut v0.5.11 and stopped where the tooling stops.** `bin/docket
...
**Which commit.** The cut arrives in `#1003`, opened as a draft at the
branch's first push so that it could not be merged before the cut was pushed
(`PL-H14W`). Its squash merge's subject ends `(#1003)`, which the second
command below resolves. If it resolves nothing, `git tag` fails on an empty
ref rather than tagging the wrong commit.

```bash
git fetch origin main
git tag -a v0.5.11 "$(git log origin/main -1 --format=%H --grep='cut v0.5.11.*(#1003)$')" -m "v0.5.11"
...
in the tree reports it either, so `git ls-remote --tags origin v0.5.11` is the
only answer.

**Done when.** `git ls-remote --tags origin v0.5.11` resolves, on the merge
commit of `#1003`.
