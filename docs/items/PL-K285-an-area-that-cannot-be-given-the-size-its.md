---
id: PL-K285
title: An Area that cannot be given the size its Editor needs says so rather than collapsing it, which is the second of the three properties docs/interface-provenance.md records Blender not supplying
status: blocked
...
**Problem.** An Area that cannot be given the size its Editor needs says so rather than collapsing it, which is the second of the three properties docs/interface-provenance.md records Blender not supplying
...
**Done when.** The Editor contract says whether a declared minimum is a request
...
size its Editor needs and asserts the stated behaviour rather than a zero-extent
pane.
