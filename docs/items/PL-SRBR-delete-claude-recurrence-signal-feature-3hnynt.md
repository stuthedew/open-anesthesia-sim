...
**Done 2026-09-23.** `git ls-remote origin
refs/heads/claude/recurrence-signal-feature-3hnynt` printed nothing: the branch
is gone from origin, and `#794` holds every line of its one commit. Closed by
`PL-WNCT`'s session, which found it deleted while re-checking before it named
the deletion again.
