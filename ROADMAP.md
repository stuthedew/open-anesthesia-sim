...
Found while building `PL-B8MK` (a bookmark is a forkable instant), which is
Required scope of this milestone. It is `safety`-classed and so re-enters this
gate whatever its presence answer, under the second of "The gate is a snapshot"'s
two unconditional exceptions. It is not itself inside Required scope, so it
clears before implementation begins, and it is `S`.
