...
## Current baseline: v0.4.9

v0.4.9 is the release in which the apparatus that checks this project was
audited against what it claimed to check, and repeatedly found to be
checking less. A guard that read nothing from the object it judged, an
independence check a plain `import` walked past, two required tests nobody
had written, a citation check that never opened the queue, and a quality run
that had been red on `main` for three merges without saying so - each of
these passed, and each stood for a guarantee that was not being kept.

Nothing a clinician sees moved. No equation, parameter, unit, numerical
method or solver step changed, and the published comparisons pass against the
same arithmetic that validated them in v0.4.7. The agent and reference-patient
files did change, and only in what they say about their own numbers: each
source now carries a machine-readable `tier` and `adopted` flag, and each file
a `provenance_gap` stating which parameters rest on a reference implementation
rather than a measurement. Not one stored value moved. The one adoption that
audit forced - `venous_pool_volume_l` on a tier-2 source - is recorded with
what it owes rather than quietly taken.
