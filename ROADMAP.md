...
v0.5.11 is the release where the in-flight guards started answering who holds
an item from the recorded claim rather than from commit subjects, and the
release train became something a claim holds. Sixteen items, all apparatus and
documentation, and a patch on both halves of § "Versioning decision"'s test: no
capability boundary is crossed, because nothing a learner can reach changed at
all, and every number above it is spent, v0.6.0 through v0.9.0 being given to
milestone sections. Two of the sixteen are entries in Gate 2, the debt gate
frozen under v0.6.0 - `PL-KNHX` and `PL-MFM4`, both in its workflow lane - and
the gate stands at 48 of its 186 entries cleared at this cut, against 46 of 185
at the last. The 186th is `PL-WMCJ`, a `science` finding captured on 2026-09-24
and placed on the frozen list because a `science` finding joins it whenever it
is made. Eight - `PL-331V`, `PL-4RK2`, `PL-DDYD`, `PL-H14W`, `PL-J9S0`,
`PL-MB3F`, `PL-N162` and `PL-Y1W0` - are v0.6.0 deferrals.

**Nothing in the simulator moved, and this cut says so by tree identity
alone:**

| Tree | v0.5.10 | This cut | What moved |
| --- | --- | --- | --- |
...
| `docs/MODEL.md` | `fd139ea` | `fd139ea` | nothing |
| `README.md` | `9938368` | `9938368` | nothing |
| `.github/` | `275baf5` | `2e0a3e2` | `quality.yml` gains one step, `uv run python tools/possessive_section_check.py`, and the comments that say why (`PL-MB3F`) |

Everything this release moves is the queue tool, the repository's own checks
and the instructions that describe them: `subprojects/docket/` (one new module,
`arming.py`, with `checks.py`, `claims.py`, `cli.py`, `model.py`, `plan.py`,
`release.py`, `render.py` and `vcs.py`, its README and seven of its test
modules), `tools/` (`branch_id_check.py`, `contrast_check.py`, `doc_check.py`,
`possessive_section_check.py`, `pr_body_check.py` and `ruff.toml`) with seven
test modules under `tests/unit/`, `.github/workflows/quality.yml`, `Makefile`,
the first-edit hook, `CLAUDE.md`, `.claude/rules/instruction-writing.md`, the
`docket` skill's capture, release and start modes, the `tools/` map in
`docs/ARCHITECTURE.md`, `docs/maintainer.md`, `docs/resident-instructions.md`,
ten recovered bodies under `docs/pr-bodies/`, and this file.

### Who holds an item: the guards read the record

v0.5.10 wrote the claim record and left `claim` and `yield` as its only
readers. Five items put the rest of the in-flight machinery onto it, and a
sixth makes the release train a thing a claim can hold.

- **`PL-N162` (`#1002`) moves `show`, `flight`, `next` and the session-start
  digest onto `claims.holdings`**, the one reader of who holds an item. Each
  hold carries a kind - a claim, a status disposition, a branch named for its
  item, a cut - and a state - live, lapsed or released - and every command
  prints both beside the id. `flight` gains a section for lapsed claims, a
  `legacy refs: N` line and an `unclaimed:` row for a session branch carrying
  work and no claim, and on this repository went from 4 rows to 9. It also fixes
  two defects in the reader: in a truncated clone a merge of `main` no longer
  releases a live claim, and `GitRunner`'s memo no longer reads a
  `-c key=value` argument as the git subcommand. `PL-FFR0`, in
  the same pull request, moves `branch_id_check`'s unclaimed-work predicate
  into `claims.py`, so CI's refusal and `flight`'s row read one definition.
- **`PL-J9S0` (`#991`) has CI refuse what the record says is wrong.**
  `tools/branch_id_check.py` fails a session branch carrying queue work and
  no claim, printing the `bin/docket claim` command, and a claim that orders
  behind another live claim, naming the holder and the yield or takeover
  command; the first-edit hook asks the same question once per session. Three
  readings of the brief were settled on the way (project owner, 2026-09-24,
  ratified, over the spec's literal "no live claim" and "outside `items_dir`",
  put to them on #991): a claim counts in any state, because a finished branch
  releases its own; the queue is the items with `ROADMAP.md` and
  `docs/WORKING_NOTES.md`, because a queue workflow writes all three; and a branch
  named for its item holds it by the name.
- **`PL-H14W` (`#992`) and `PL-DDYD` (`#993`) keep a merge from erasing a
  claim.** v0.5.9's `#978` opened as an ordinary mergeable pull request
  carrying only a capture and a start claim, and was merged by hand six minutes
  later while the cut it was claimed for was still uncommitted, erasing the
  claim with the branch. `PL-H14W` opens such a pull request as a draft, which
  GitHub will not merge, until the push that closes or blocks the item (project
  owner, 2026-09-24, ratified, over opening none while a claim rides and over
  titling it as the capture). `PL-DDYD` replaces the catalog of claim shapes
  `CLAUDE.md` applied by judgment - every shape it missed had merged a claim
  away (`PL-QP9Z`, `PL-1MCK`, `PL-KWCY`) - with `bin/docket arm`, in the new
  `arming.py`: it reads the branch's net diff, the claims bound to it and how
  far behind the base it is, and answers `arm`, `hold`, `behind N` or
  `unknown`. `CLAUDE.md` now names the command instead of restating the
  catalog, and the catalog's ratified decisions moved into the module's
  docstring.
- **`PL-331V` (`#995`) makes the release train a claimed resource**, and
  closes `PL-MFM4` with it. A release guard that read only refs carrying a cut
  could not see a session that had filed a release item and asked for the
  version without cutting, so `PL-Z0C7` and `PL-R5VS` were two ids for one
  release. A release item now carries `resource: release-train` and its claim
  holds the train: `bin/docket new --resource release-train` writes nothing
  while another claim holds it, `bin/docket release` refuses a rival holder,
  another ref's cut, or a branch holding no train claim of its own, and the
  digest names the rival instead of offering a release. This cut is the first
  taken under it, as `PL-DRRG`.

**`PL-MB2W`, the generator head the record answers, stays blocked, now on
`PL-FX5Q` alone.** Seven of its eight blockers are done, and `PL-FX5Q` deletes
`vcs.precedence`, `Carrier` and `branches_in_flight`, which nothing calls any
more. Commit subjects are still read inside `claims.holdings` for commits from
before the cutover, until `flight` prints `legacy refs: 0` and `PL-CH3Z`
removes that reading. While `PL-MB2W` stays open with `generator: live`, the
pause on new workflow mechanisms holds.

### The permanent record of a merged pull request

- **`PL-25JP` (`#994`)** ran `tools/pr_body_check.py --recover` for the ten
  squash commits on `main` that had landed with no body - `#848`, `#849`,
  `#886`, `#887`, `#910` to `#913`, `#916` and `#918` - writing each pull
  request's body verbatim into `docs/pr-bodies/`, so the digest line that named
  them in every session is gone.
- **`PL-Y1W0` (`#998`)** adds `pr_body_check.py --compare`, because a body that
  landed non-empty but different from its pull request's - frozen when
  auto-merge was armed, edited after the merge, or replaced by the merge client
  - passed as intact. Plain equality flags 679 of 681, so it normalizes both
  sides first (the co-author trailer, the generated-by footer, whitespace and
  abbreviated hashes), and then finds 27 of 681 that differ, not the eight the
  item was filed on. It is advisory and always exits 0. Recording the 27 is
  `PL-PNJF`'s, and preventing a frozen body stays `PL-M7W1`'s.

### Checks that passed on what they could not see

- **`PL-MB3F` (`#999`).** `tools/doc_check.py` silently skipped a source file
  the running interpreter could not parse, and `app/bookmarks.py` and
  `app_metadata.py` use syntax newer than the 3.11 floor the tools are held
  to, so no gate checked their docstrings' citations. Both `doc_check` and
  `possessive_section_check` now name each file they could not read on a "Not
  checked" line - two at the floor, none under `uv`'s interpreter - and `make
  check` and CI run both under `uv` (project owner, 2026-09-24, ratified, over
  reporting the skip and keeping this line bare).
- **`PL-KNHX` and `PL-TP75` (`#996`)** are `tools/contrast_check.py`, the WCAG
  contrast check on the interface's colours. A `KNOWN_SHORTFALLS` entry whose
  requirement had been renamed or deleted was read by nothing and still counted
  as a shortfall; it is now an error. And the verdict line summed seven of the
  eight error kinds that set the exit code, so a run failing only on a chart
  trace below its contrast floor exited 1 under a header reading "0
  errors"; the kinds are now listed once, and both read that list. On this
  tree the line is unchanged: 24 of 24 declared requirements meet WCAG 2.2 AA,
  with 0 known shortfalls and 0 errors.

### Generator heads, and the commit-attribution rule

**`PL-4RK2` (`#997`)** finishes what v0.5.10's `PL-QFWF` left. A blocked live
generator head passed its rank one edge down, so a head whose only startable
work sat behind a blocked blocker, or which waited on a milestone alone, ranked
nothing and nothing said so. `plan.unranked_generators` now names those heads,
with what each waits on and the startable work at the chain's end; `bin/docket
next` prints them, and `show` and `generators` stop calling a blocked head
ranked. The chain is reported rather than followed. `PL-TS8G` was dropped in
the same pull request as a duplicate of `PL-9F8B`, which stays open.

**`PL-SL16` (`#1001`)** states `PL-B11M`'s commit-attribution decision where a
session reads it before committing: `CLAUDE.md` § "Name the work after the
item" now ends with the rule that every commit a Claude session makes ends with
the plain `Co-authored-by: Claude` trailer and never a model-named co-author,
whatever the harness's attribution reminder names, since that reminder yields
by its own terms to a `CLAUDE.md` rule (project owner, 2026-09-24, restating
`PL-B11M`). Until then, 800 of the 970 branch commits of pull requests merged
from 2026-09-17 to 2026-09-24 carried a model-named co-author. Whether the
sentence is enough is `PL-LWMS`'s to measure once 30 pull requests cut from a
`main` holding it have merged; `#1000` rewrote that item as a check of a pull
request's commit trailers, built only if the count still finds them.

### The rest, and this release's tag span

`PL-NLXK` (`#989`), the cut of v0.5.10, closes here for the structural reason
every cut does: a cut cannot stamp itself. `PL-LRP2` closed beside it, once
`git ls-remote --tags origin` showed v0.5.9 tagged on its cut's merge. This cut
recorded all sixteen `pr:` numbers before rendering its notes, so every bullet
cites its pull request. One bullet is reworded by hand: `PL-SL16`'s title
quotes the model-named trailer, which `PL-B11M` allows in no pushed artifact,
so its bullet says "a model-named co-author" instead. Whether the item's own
title and census follow is `PL-CGBK`'s.

**None of the sixteen shipped inside v0.5.10's tag.** That tag is on
`6f43a628`, the merge of its own cut, and every other merge this release
describes came after it - `git log v0.5.10..origin/main` holds all eleven - so
v0.5.10's notes take no `### also inside this tag's span` pointer. This cut is
handled as v0.5.10's was, on `PL-V065`'s precedent (project owner, 2026-09-21,
ratified, over holding the release while sibling sessions merge and re-running
`make release` under the same number): anything merging after it is taken and
before the v0.5.11 tag ships inside that tag, is described in the next
release, and takes a pointer in v0.5.11's notes at that release's cut. No other
pull request was open when this cut was taken.
...
## Completed: v0.5.0 - the case you can branch
...
- PL-LHBY (M) — **added 2026-09-20, `safety`.** `SimulationView._refresh_bookmarks`
  draws the marks panel from the reference run alone, and a mark's *standing* is
  per-run where its *set* is not. With a branch displayed, a branch halted on a
  learner's MAC target shows no standing at all - the halt is indistinguishable
  from a Pause - and a mark the trunk reached is rendered `reached` beside a
  branch whose own standing says it never can be. `safety`: `CLAUDE.md` names the
  correct value under the wrong patient context as a failure of the value, and a
  standing is a claim about which run got there.

  **Admitted although the problem postdates the freeze by a fortnight**, because
  the rule below makes `safety` not deferrable and because this gate's own
  Required scope is what created it: `PL-VKJW` made two runs reachable from a
  shipped entry point for the first time. It is therefore debt this milestone
  clears itself rather than a precondition to beginning it, which is the only
  reading under which a defect created by a milestone can sit in that
  milestone's gate at all.
...
**Added 2026-09-20 under the unconditional safety/science exception — 3 entries**

All three were captured on 2026-09-19 or 2026-09-20 and triaged on 2026-09-20
(`PL-WQF1`). Each is `safety`- or `science`-classed and so re-enters this gate
whatever its presence answer, under the second of "The gate is a snapshot"'s two
unconditional exceptions. None is inside this milestone's Required scope, so all
three clear before implementation begins, and all three are `S`.

The two `science` entries are the rest of `PL-4DCG`'s machine survey reaching
`docs/MODEL.md`: that survey read the anesthesia machines in current clinical
use against what this model claims, and what it found was mostly statements the
document owes about its own limits rather than defects in the arithmetic. The
`safety` entry is `PL-JVHL`'s successor, one axis over.

- PL-0RZ0 (S) — **added 2026-09-20, `safety`.** `PL-JVHL` took the *run* out of
  the pointer's hands, and deliberately left distance settling which
  **compartment** answers. The cause it named is untouched: the percent axis is
  scaled by the alveolar peak, so two slow compartments of one run contend
  inside one hover radius exactly as two runs' copies of one compartment did.
  Measured incidentally while closing `PL-JVHL` — at 20 minutes on the branched
  sevoflurane case the trunk's and the branch's fat values land on the same
  pixel row of a 480 px plot. `safety` for `PL-JVHL`'s reason: the box prints a
  compartment name and a value, fat and muscle at one instant can differ
  severalfold, and a hand movement too small to aim with changes which of them
  is being read. **The scan comes before any rule** — `PL-JVHL`'s own lesson is
  that two of its three candidate rules changed nothing once counted — and the
  honest outcome may be to record the measurement and drop this in favour of
  `PL-QYBW`, which is the axis underneath it and is declined to Gate 2 below.
- PL-D126 (S) — **added 2026-09-20, `science`.** This project treats Gas Man as
  its reference implementation and compares against it throughout
  `docs/MODEL.md`. Candries, De Wolf and Hendrickx's prospective in-patient
  validation (*J Clin Monit Comput* 2022;36(6):1881–1890) reports Gas Man's
  agreement split by phase: over the first 15 minutes MDPE 18% and MDAPE 21% on
  the FLOW-i and 7% and 13% on the Zeus, against 0% and 6%, and −1% and 5%, over
  the last 45 minutes. The authors state the model performs better for
  maintenance than for wash-in. The wash-in is the phase this simulator most
  displays, so the phase in which its reference is least accurate is the one it
  shows most, and § "Known limitations" does not say so. A statement about the
  limits of a comparison rather than a defect in this implementation.
- PL-WJNS (S) — **added 2026-09-20, `science`.** § "Breathing circuit" states
  that "an equal fresh-gas volume leaves through the exhaust at the current
  mixed-circuit concentration", and § "Circuit exhaust" and the mass-balance
  identity are built on that being unconditional. It is how a semi-closed circle
  behaves and not how every machine behaves in every mode: Meyer et al. record
  that on the Zeus "the surplus gas valve can be closed to prevent any loss of
  gas volume", which is not a small exhaust but a different conservation
  statement. Confirmed at triage that `docs/MODEL.md` carries no semi-closed
  assumption anywhere — § "Model boundary" and § "Assumptions" are silent — so a
  reader who knows closed-circuit machines exist cannot tell whether the model
  excluded them or overlooked them. It matters ahead of any machine
  implementation for the reason the survey gives: this is the one machine
  variable that reaches an *equation* rather than a coefficient.

**Added 2026-09-20 under the unconditional safety/science exception — 2 entries**

One problem and its consequence. The first was found while building `PL-B8MK`
(a bookmark is a forkable instant), which is Required scope of this milestone;
the second while deciding the first, later the same day. Both are
`safety`-classed and so re-enter this gate whatever their presence answer, under
the second of "The gate is a snapshot"'s two unconditional exceptions. Neither is
itself inside Required scope, so both clear before implementation begins, and
both are `S`.

The problem predates `PL-B8MK` in the code and not in reach: the comparison it
turns on has always been strict, and until a bookmark was forkable a mark
sitting exactly on a branch's fork instant needed the learner to have marked a
control-event instant by coincidence.

- PL-3K9B (S) — **added 2026-09-20, `safety`; decided 2026-09-20.** A branch
  ...
  branch whose identity is the management taken at that height. Four further
  cases were then measured, two of them on a trunk with no branch anywhere - a
  mark added behind a running clock, a fork taken at a control event, a mark
  re-added at a fork instant, and a branch `reset()` - so the root is that
  `still_running` is inferred from absence from the halt set where a one-way
  clock asks whether the instant is still ahead of the run. Decided (project
  owner, 2026-09-20, ratified, over reusing `REACHED` for the at-or-behind case
  and over the item's original seed-and-reorder answer): add `PASSED` as a
  fifth `MarkStanding` and decide a time bookmark from the run's own clock,
  `REACHED` keeping its meaning and its word for MAC targets. `PL-4KZD` below
  is what that decision unmasks.

- PL-4KZD (S) — **added 2026-09-20, `safety`.** The bookmark panel is drawn once
  from `snapshots[0]`, so while two runs are shown it states the reference run's
  standings for a set every run carries, with nothing on the row naming a run.
  `_refresh_bookmarks` justifies the single read from the marks being shared,
  which is true of `snapshot.bookmarks` and false of
  `snapshot.bookmark_standings`: a standing is computed from that run's own
  clock, its own `began_at_s` and its own halt set. Measured 2026-09-20 on a
  trunk halted at 0.4 ×MAC reading `reached` while the branch forked there reads
  `still_running`. Captured after the freeze and re-entering under the
  unconditional `safety` exception; the problem is older than the freeze, the
  panel having read one snapshot since two runs could be drawn at all. It is
  masked while a standing is close to a permanent latch, and `PL-3K9B`'s decision
  unmasks it by making the standing answer where each run *is*.

**Added 2026-09-20 under the unconditional safety/science exception — 1 entry**

- PL-N3N5 (S) — **added 2026-09-20, `safety`.** `_bookmark_standings` passes
  `stopped_at_cap` and nothing about whether the run can take another step, so
  every mark on a *failed* run reports `MarkStanding.STILL_RUNNING` — which says
  in its own docstring that "the run can still reach this mark, and has not
  yet", while `SimulationController.start` refuses that run outright ("cannot
  resume a failed simulation … reset it first"). Found by the adversarial review
  of `PL-LHBY` and reproduced by failing one of two displayed runs: the run's own
  panel read `Stopped — simulation error` while its mark clause claimed the mark
  was still ahead of it. Captured after the freeze and re-entering under the
  unconditional `safety` exception; the problem is older than the freeze and
  older than `PL-LHBY`, the standing having been wrong since `PL-CTD7` and
  invisible only because `STILL_RUNNING` rendered as silence. What `PL-LHBY`
  changed is that the standing is now drawn in words on an attributed row, where
  a learner can read it.

**Added 2026-09-21 under the unconditional safety/science exception — 1 entry**

The third `safety` finding from the adversarial review of `#784`, seated in the
triage pass of 2026-09-21; `PL-LHBY` above is the first and was admitted on
2026-09-20 under the same exception. Its three feature-mates from that review
are not here: one is deferred below, and the other three are classed `test`
alone, which is not a debt class and so owes this gate no disposition.

- PL-25DD (S) — **added 2026-09-21, `safety`.** The wash-in state line and the
  off-scale notice are placed one per run into two shared columns beside the
  chart — `_place_run`'s `_off_scale_column` and `_wash_in_state_column`
  holders — and neither text carries a run. `WASH_IN_STATE_TEMPLATE` is
  `"Now: F_A/F_I = {ratio}"`; `off_scale_notice` names compartments and a
  ceiling. With a branch drawn, two unlabelled lines stack, and the only thing
  separating them is stacking order, which nothing on screen declares while the
  run names sit in a different splitter. `safety`: `CLAUDE.md` names the correct
  value under the wrong patient context as a failure of the value, and F_A/F_I
  read off the wrong run inverts the comparison a branch exists to teach.

  **Admitted although the problem postdates the freeze**, for the two reasons
  `PL-LHBY` was: the rule below makes `safety` not deferrable whatever the
  presence answer, and this gate's own Required scope is what created the
  hazard — `PL-VKJW` made two runs reachable from a shipped entry point for the
  first time. The widgets are older than the freeze; their ambiguity is not.

**Added 2026-09-21 under the unconditional safety/science exception — 2 entries**

Both found while implementing `PL-25DD` above, not by the `#784` review: fixing
two of the six shared layouts `_place_run` distributes a run's widgets across
is what prompted reading the other four, and two of those carry the same gap.
They are admitted on `PL-25DD`'s own two reasons — the rule below makes `safety` not
deferrable whatever the presence answer, and this gate's own Required scope is
what created the hazard, `PL-VKJW` having made two runs reachable from a
shipped entry point for the first time. The widgets predate the freeze; their
ambiguity does not. The other two of those four need nothing:
`build_transport_row` already carries `_run_name_text`, and the header badge
names the agent.

- PL-TSZM (S) — **added 2026-09-21, `safety`.** `_holder(self._notice_column,
  (run.build_notice(),))` puts one bare `NoticeLabel` per run into a column the
  runs share, and all three notices it carries assert something about a run and
  name none: `FAILURE_NOTICE_TEMPLATE`'s "Simulation stopped",
  `SUPPORTED_LIMIT_NOTICE_TEMPLATE`'s "this run reached the supported run
  length", and `REFUSED_SETTING_NOTICE_TEMPLATE`'s "The simulation is unchanged
  and still running its previous setting". `safety`: the last is not merely
  unattributed but *false* of the run it is not about, and the hazard table's
  existing row for reading a failed halt as a pause spends the distinction it
  buys the moment two runs are drawn.

- PL-C3GS (M) — **added 2026-09-21, `safety`.** `_holder(self._sidebar_column,
  run.build_sidebar_panels())` puts *two* panels per run into a shared column,
  so a branch makes the sidebar read Agent accounting validation / Control
  changes / Agent accounting validation / Control changes with no run named on
  any of them. `CONTROL_TIMELINE_CAPTION` is "What was changed during this run,
  most recent first" — "this run" with nothing saying which, over settings and
  the simulated times they took effect. `safety`: comparing two managements of
  one patient is what a branch is for, so a setting change read off the wrong
  panel says the trunk received an intervention it never got. `M` rather than
  `S` because the headings are built inline in `build_sidebar_panels` rather
...
adversarial review of `#784` as `PL-25DD` above and `PL-LHBY` before it, and
both defer where those two are admitted. The difference is the rule rather than
a judgment about severity: neither is classed `safety` or `science`, so neither
carries the unconditional re-entry, and the presence test they are then held to
answers no. `git log -S` dates both symbols to `d048aca` (2026-09-20), the pull
request that built the fork control — work done while implementing the milestone
this gate protects, which is the case the rule names in terms.

- PL-WG73 (S) — **deferred 2026-09-21.** `COMPARING_FORK_LOCK_TEXT` sends the
  learner to "Reset the case" where the only Reset label in `app/` is `"Reset"`,
  and once a branch is drawn the two Reset buttons do opposite things: the
  trunk's ends the comparison, the branch's — the one the learner reaches for —
  discards everything simulated since the fork and leaves the lock standing.
  `defect`, `ux`. It costs the learner their branch's progress and a mode they
  cannot see the way out of; it displays no wrong clinical value, which is why
  it sits at `P2` and outside this gate.

- PL-624C (S) — **deferred 2026-09-21.** `test_the_branch_is_drawn_beside_the_trunk_on_one_time_axis`
  calls `view.present(False)` itself between the fork and the frame it reads, so
  the frame it inspects is the one it asked for and a fork that failed to redraw
  would still pass it. `defect`, `test`. A check passing while its guarantee is
  void is the shape this project treats as compounding friction, but the
...
The arithmetic is the argument. On that date Gate 1 stood at 132 entries
against Gate 0's 21 — already by a wide margin the largest this project had
held — with 89 still open. Admitting all forty-two would have taken it to 173
and grown it 31% at a point where it was not draining, which is the shape
Phase 0 was retired for: a gate that refills faster than it drains is abandoned
rather than followed, and that is worse than not having one.

Those four numbers are the recorded arithmetic of the 2026-09-08 decision, held
to that date and not to the tree, so they are never recomputed: rewriting them
to today's figures would restate the reasoning in numbers the decision was not
taken on. They are not a current gate count and must not be read as one —
`bin/docket wave` is what says where the gate stands now.
...
- PL-09G9 (M) Nothing refuses a new verify: command that re-runs a test file make check already collects, so PL-6TP8's contract is enforced by prose alone and 82 of 180 open commands carry the clause — closed in v0.4.29
- PL-0M32 (S) Nothing can tell an item finished under a renamed test from one nobody has started — closed in v0.4.28
...
- PL-3V6C (S) PL-TFWR and PL-XQRK record incompatible causes for the same remote-deletion failure and both ruled out the git proxy on a field that does not record policy denials, so whichever lands first writes an unproven cause into the instructions — closed in v0.4.28
- PL-483K (S) The stop hook demands a push for work already pushed after a branch is restarted per the merged-PR recovery, because checkout -B from origin/main leaves the upstream pointing at main — closed in v0.4.14
- PL-4FBP (M) Fifteen items repair a document sentence whose link to the tree lives only in the reader's head: decide whether an assertion must name what it asserts — closed in v0.4.28
- PL-4Q9B (M) Ten items work around the clone being trusted as the remote and around an unrecorded set of permitted ref operations: record both — closed in v0.4.28
- PL-69JZ (S) docket verify's 'the checks themselves are unedited' audit REJECTs every item whose declared work is editing a .claude rules file, since gate_paths includes .claude and touches is not consulted — closed in v0.4.14
- PL-6BDX (S) Two shipped items are reported in flight in every session's digest, because their branch refs outlived their merges: PL-GVXP (v0.4.7) and PL-S5LB (v0.4.6) — closed in v0.4.15
...
- PL-6TN8 (S) A verify: command whose discriminating half greps for a test name can exit 1 because the name was guessed, not because the work is outstanding, so watching it fail proves less than the rule assumes — closed in v0.4.28
- PL-6TP8 (M) Twelve items re-decide what a verify: exit status proves, because the field was specified as a command string and nothing else: one contract rather than twelve patches — closed in v0.4.28
- PL-6YWK (S) PL-4L6Z's verify: runs the whole reference suite, so it is killed at docket check's 120s limit and nothing is claimed about the item on any run — closed in v0.4.28
...
- PL-77SV (S) PL-G8TR, PL-K2C8 and PL-TFWR all edit the same ten lines of the docket skill's recovery block, and PL-G8TR's Done-when is defined against that block's shape, so whoever lands first silently sets the other two's tests — dropped 2026-09-19
- PL-7RTN (M) An open pull request can carry no check runs at all, so a branch merges with nothing having gated it
- PL-7SVX (S) Delete spikes/ and the last Flet import once the port is complete — closed in v0.4.26
- PL-7XTS (S) Nothing routes a close-out to bin/docket verify --self, so a session auditing its own branch runs the delegated mode PL-69JZ fixed and still gets a REJECT on correct work — closed in v0.4.26
- PL-879R (S) docket check advises on a verify: command's outcome but never its shape, so a grep for an id, or for a file another item is known to create, is only reported once it has already started passing — dropped 2026-09-17
...
- PL-BHVM (M) Nineteen items re-decide what evidence proves a ref is done, seventeen of them in vcs.py: one design round rather than nineteen heuristic patches — closed in v0.4.28
- PL-BQ46 (S) MacAwakeReference.fraction_of_mac and formatting.mac_multiple are a third and fourth dimensionless convention beside Fraction, and neither is distinguished from a concentration fraction at the type level — closed in v0.4.24
- PL-C92D (S) PL-YSZN's Flet frame table predates PL-2FM6 and measures a tree that no longer exists in two of its three stages, so the Qt/Flet comparison rests on one row — closed in v0.4.26
...
- PL-F48B (S) Nothing ever repairs a clone's tags after a history rewrite: fetch_remote runs git fetch without --tags --force, so release tags keep pointing at purged commits — closed in v0.4.28
- PL-F933 (S) doc_check resolves a path citation against the working tree, so a citation to a gitignored path passes locally and reddens CI — closed in v0.4.26
...
- PL-HWW1 (M) Three items patch a reading of ROADMAP.md's Required scope because membership is cited rather than declared: make the declaration the record — closed in v0.4.28
...
- PL-KL2Q (S) core/parameters.py now holds two overlapping percent/fraction vocabularies - Pydantic's PositivePercent which validates but does not type-check, and concentration.py's Percent which type-checks but does not validate - and nothing says which a new field takes — closed in v0.4.24
...
- PL-L09X (M) An item blocked on a milestone that is decided but not yet named has no honest status: bare blocked errors, and blocked-by only accepts a version the roadmap already places — closed in v0.4.14
- PL-LBR6 (S) bin/docket record renames a drifted item file as a side effect of writing a pr number, which conflicts against whoever else is holding that file — closed in v0.4.29
...
- PL-LKGL (S) A verify: command cannot detect a stale item: it tests for the presence of the fix, not the fault, so an item whose problem was solved another way stays red forever and reads as outstanding work — closed in v0.4.28
- PL-LN3T (S) wave does not report a milestone row the version has released whose section's Required scope is still open, so a patch cut at a milestone's own number leaves the plan stepped past it once the hand-off has scrolled by — closed in v0.4.28
...
- PL-NF6N (M) Triage has no write command: every pass edits item front matter by hand, which is the decidable half CLAUDE.md asks to be moved into code — dropped 2026-09-19
...
- PL-PQQ2 (S) PL-KBD0 is the workflow lane's top pick but all three live instances its brief names are now closed, and no open item at ready or needs-decision carries a blocked-by field — dropped 2026-09-19
...
- PL-T7VS (M) A red doc_check voids the whole-store verify replay for the 29 open items gated behind it, and the replay reports green rather than declining to answer — dropped 2026-09-19
...
- PL-TNB6 (S) docket next attributes the whole of Gate 1 to the v0.4.x step, where the plan makes Gate 1 its own row that no patch may ship — closed in v0.4.24
- PL-TP5M (S) PL-ZBRB's advisory names blocked-by to work around a checker that did not read it, and PL-KBD0's check now does
- PL-TTMF (S) PL-6BDX's title names two expired refs and the live instance this item was filed on has cleared too, so the item now reproduces nothing while the defect it describes is unfixed — dropped 2026-09-19
...
- PL-WZBX (M) bin/docket wave counts the four entries ROADMAP.md's gate places under 'Cleared by v0.5.0 itself' as clearable before the milestone begins, where the gate rule says the milestone clears them — closed in v0.4.24
- PL-XMNC (M) The pull-request verify replay scopes to items whose item file the branch edited, so a branch that invalidates some other item's verify: command by editing the file that command reads replays nothing, and the break is reported only by the whole-store sweep after the merge — closed in v0.4.28
...
- PL-Y31G (M) bin/docket stranded classes a branch left on pre-rewrite history as one whose pull request merged, because it compares file content and a rewrite leaves content unchanged — closed in v0.4.15
...
gate is a snapshot" says a finding "defers to the next gate only when the
problem itself is new: introduced by work done while clearing this gate or
implementing the milestone it protects", and both are properties of the
attributed mark row that item created. `PL-FPY2` is the one to watch and is
deferred with that said: a wrapped row's continuation can now be a whole clause
naming a run, so it can read as a row of its own and bind one management's
answer to the wrong mark — measured at a 350 px sidebar in the real 12 px font.
It is declined only because the rows it affects are the ones `PL-LHBY` made
correct in substance, so the reading is harder rather than wrong, and it is
recommended as the first of the two taken off this list. `PL-08CR` is a
`needs-decision` tidy-up: two public row formatters that no production path
calls since that same change, with their docstrings already corrected so
nothing in the tree is misleading. The two: `PL-08CR`, `PL-FPY2`.

**`PL-RBMK` is a third from the same close-out and declines on the
refilling-queue ground instead**, the problem predating the freeze rather
than being new: `SimulationView._apply_to_every_run` has always documented a
two-phase write it does not implement. It is inert — every `BookmarkSet`
operation is a pure function of the set and the mark, so with the runs' sets
equal all accept or all refuse at the same run, and the same review fuzzed
that invariant over roughly 24,000 randomized dashboard gestures without a
single mismatch. What changed is only the consequence if it ever broke:
`PL-LHBY` made the panel read every run's standings against the reference
run's set, so a divergence now halts the dashboard instead of quietly drawing
the trunk's answer. A docstring nobody can act on wrongly today, guarding an
invariant that now has teeth.
...
**Twelve more from the 2026-09-20 triage pass over the twenty-two captures open
that day** (`PL-WQF1`). Nine are wholly in the workflow lane and sit exactly
where the first thirty do: every one was captured on 2026-09-19 or 2026-09-20,
well after the 2026-09-06 freeze, all are `P2`, none is `safety` or `science`,
and all are apparatus held to `.claude/rules/apparatus-standard.md`'s
deliberately lower bar. They are `PL-BHBZ` and `PL-HJZW` (two checks whose
stated severity does not match what they do), `PL-GJPD` and `PL-WG7Q` (the two
halves of how a closure's pull request is recovered, worked in that order),
`PL-MFM4` and `PL-W7WL` (two ways `bin/docket release` loses to a second
session or to a late merge), `PL-NWSK` (a fifth refusal the required-checks
reconciliation does not know about), `PL-RR1N` (a `verify:` phrase a document's
own line-wrapping can split) and `PL-VJPJ` (a traceback on a closed pipe). The
refilling-queue arithmetic in the four paragraphs above is unchanged and is
still the reason: this gate holds three open entries and is nearly clear, and
admitting nine apparatus items that cannot reach a reader of the simulator
would reopen it days before the milestone it gates begins.

Two of the nine are worth a sentence because they fired on this pass itself.
`PL-HJZW` is why this subsection has entries at all in the shape it does - the
gate-disposition failure prints under the advisory heading, which says judgment is needed, while
`tests/unit/test_doc_check.py` makes it a hard `make check` failure, so the
pass that creates the advisory is the pass most likely to read the label and be
wrong. `PL-VJPJ` is the traceback a session gets from `bin/docket next | head`.
Neither is more urgent for having been met here; both are recorded so the next
session meeting them knows they are known.

**Three are not wholly apparatus and so need their own ground**, which is stated
rather than stretched.

`PL-LPH9` reaches `ROADMAP.md`, which is product direction. It is `needs-decision`
because this document states a cadence rule - "no interim release is cut partway
through clearing" a gate - that the last twenty-seven cuts do not satisfy, and
which of the two is the record has to be settled rather than assumed. It is
declined because it is a correction to this document's account of its own
release train and cannot reach a reader of the simulator, which is the test the
first twenty-nine were declined on, and the ground `PL-0VFF`, `PL-880Z` and
`PL-DHJ7` are recorded on above. Worth naming, because the rule it questions is
this section's own neighbourhood: whichever way it lands, no gate entry changes.

`PL-JQY1` reaches `src/anesthesia_sim/` and `docs/MODEL.md` and so could reach
such a reader. It is declined because what it was captured for is already done
and nothing misleading is live: all three agent data files carry an
`adopted: true` source note, in place since 2026-08-23, saying that the stored
maximum "is a device-capability limit on the delivered-concentration control,
not a scientific model parameter", naming the Tec 6, the Vapor 2000 and Sevotec
5, and the Sigma Delta and Isotec 5 respectively. What is left is the structural
half - a device constant keyed under the agent while
`src/anesthesia_sim/data/machines/` exists - which produces no wrong value under
either answer. That is the ground `PL-BQ46` and `PL-KL2Q` were declined on: a
naming-and-placement decision, correct today whichever way it goes.

`PL-QYBW` reaches the chart a learner reads, and is the mechanism under
`PL-0RZ0`, which is **admitted above** under the unconditional safety exception.
It is declined on the ground `PL-5B1N` is recorded on: nothing is misleading
anybody meanwhile. The shared percent axis is honest - labelled, correctly
scaled, and the hover box gives exact values - so what the item describes is a
legibility and teaching-value shortfall rather than a wrong or mislabelled
value, which is why it is `ux`/`defect` and not `safety`. The safety obligation
it records is about its own *answer*: a second axis or a split scale that does
not announce the change of scale would be the misleading visual encoding
`CLAUDE.md` names, and that binds whoever builds it. It is `needs-decision`
because choosing between the four options changes what a learner sees, which is
the project owner's call rather than a session's.

**One consequence of that split is worth writing down**, so a later session is
not surprised by it: `PL-0RZ0`'s own "Done when" allows closing it by
measurement - if the share of contended hovers proves small, the honest outcome
is to record that and drop it in favour of `PL-QYBW`. A gate entry closing as
`dropped` with a reason that names deferred work is an allowed disposition and
not a hole; what would be a hole is closing it silently.

- PL-BHBZ (S) `bin/docket verify`'s suppression check reads every added line regardless of file type, so prose naming `xfail` in `ROADMAP.md` or a release note REJECTs a correct close-out
- PL-GJPD (M) `_carried_work` reads a combined commit as having carried the work, so a triage pass that also edits `ROADMAP.md` is recorded as an item's pull request
- PL-HJZW (S) `tools/doc_check.py` prints the gate-disposition failure as an advisory while `tests/unit/test_doc_check.py` makes it a hard `make check` failure
- PL-JQY1 (S) `max_delivered_concentration_percent` is a vaporizer device maximum keyed under the agent, with `src/anesthesia_sim/data/machines/` already built for the class
- PL-LPH9 (S) `ROADMAP.md`'s cadence forbids an interim release partway through clearing a gate, and the last twenty-seven cuts did exactly that
- PL-MFM4 (M) `bin/docket release`'s collision guard reads refs for a cut in progress, so it cannot see a second session that has filed a release item and cut nothing
- PL-NWSK (S) `required_checks_check.py` treats a paths-filtered `pull_request` job as reporting, where GitHub leaves such a check pending forever
- PL-QYBW (M) The chart's percent axis is scaled by the alveolar peak, so the slow compartments are compressed into 1-2 px
...
- PL-VJPJ (S) `bin/docket next` raises `BrokenPipeError` and prints a traceback when its output is piped into a command that closes early
...
**Five more from that item's close-out, 2026-09-20, on the same ground.** Its
work landed in #797, and its own audit turned up three findings, the design
round its decision did not reach, and one gap that the close-out's own red CI
exposed. All five are apparatus, none is `safety`- or `science`-classed, and
admitting them would grow a gate standing at 177 of 178 done at the beat where
the milestone is one step from starting.
...
- PL-J3WK (S) `make check` runs `bin/docket check` without `--verify` while CI runs it with, so a `verify:` command that proves nothing passes every local gate and fails CI - and both halves of what fired are decidable from the store alone
...
- **Time bookmarks and MAC targets, as two separately listed collections**
  ...
  does. **Shipped in v0.4.33, with two deviations from that floor recorded
  here rather than silently taken.** The height is held as a multiple of the
  running agent's 1 MAC rather than as a percent of it - the unit every
  compartment is already read in, so a target is compared against the readouts
...
- **A bookmark is a forkable instant** (queue items `PL-B8MK` and `PL-TYWQ`),
  so a branch can be
  ...
  costs the trunk nothing and is exact too. **Shipped 2026-09-20 on that
  second route**, as `SimulationController.resumed_at_halt` and
  `BranchedCase.fork_at_halt`: no keyframe is recorded anywhere, and the trunk
  is element-for-element the case it would have been unmarked. It was behind
  `PL-LPLD` and `PL-CTD7`, which build the bookmark and the halt it is taken
...
**Frozen 2026-09-21, the day v0.5.0 shipped.** This milestone was scoped out of
...
v0.4.0 and v0.5.0 sections both give: a count written into a document goes
...
want a session that can hold the whole change.

**`PL-Y04W` is deferred to Gate 3 on § "The cadence" beat 3's terms, and the
three conditions are met here rather than assumed.** It is the break-out build
item - an Area taken into its own top-level window, the window lifetime that
keeps the main window unclosable while break-outs exist, and the test that
every window the application can be left showing carries the invariant display
tier - and "The timeline" gives break-out to row 9, v0.7.0. It reaches this
gate because it is `safety`-classed rather than because it is ordinary debt, so
requiring it before v0.6.0 begins would require v0.7.0's own build item to
close first, which no order satisfies. So: this section says so and says why;
the deferral names **Gate 3**, which freezes when v0.6.0 ships and clears
before v0.7.0's implementation begins, which is the release that builds it; and
...
- PL-JSY5 (S) docs/MODEL.md's readout-row rung and its educational-disclaimer line both assume one window's width and one always-on-screen surface, which the area system makes false

**Deferred to Gate 3, because v0.7.0 is where the work is - 1 entries**
...
- PL-4V6B (M) Decide whether the decision archive should be separated from the work queue
- PL-4W2L (M) design round: pick the altitude for verify's assertion check before the next of its three open items is worked, since six fixes have each uncovered the next and PL-G21K's ratified decision reached only the suppression check beside it
- PL-6YYR (M) A release tag can be pushed for a version that was never cut, and nothing detects it: v0.4.8 tags main at version 0.4.7 with no release notes and no ROADMAP row
- PL-73P0 (M) default_base falls back to the literal string main when no candidate ref resolves, so every comparison in vcs.py can be taken against a guessed base with nothing in the answer saying so
- PL-7RTN (M) An open pull request can carry no check runs at all, so a branch merges with nothing having gated it
...
- PL-SY1J (M) bin/docket show named only one of two branches carrying PL-D1RT, and it was the later one, so the session holding the item read a verdict telling it to stand down
- PL-SYG4 (M) The digest's RESERVED verdict suppresses the release offer entirely rather than naming the next free patch number, so on the v0.4.x track with v0.5.0 reserved a session never offers a cut the plan actually wants
- PL-SZJ2 (M) wave reports nothing for a released milestone row whose own frozen list still has open entries, which is PL-LN3T's case in the other subsection: a gate-only milestone released with its list open drops out of the report entirely
- PL-TKFD (M) A session that discovers mid-work that its work falsifies an assertion cannot declare falsifies: on the base, so verify --self has no clean route for the case the field was built for
- PL-WNQT (M) bin/docket stranded reports a one-path commit as work left behind whenever a later merge edits that file, and its recovery recipe would revert the newer work
...
- PL-12P8 (S) PL-JBZK's lane check assumes every file under tests/ is a test file, so a shared non-test helper is told to declare itself apparatus
- PL-162Y (S) docket next names a blocked item whose item blockers have all closed, but not one whose blocked-by milestone is now scoped, which docket check reports as equally promotable
- PL-1BS2 (S) The session-start digest's Releasable line reads readiness without the interrupted-cut resume, so during an unfinished cut it reports the short remainder with nothing saying why
- PL-1T6T (S) Re-test the refusal of compaction now that root CLAUDE.md is known to reload after a compact
- PL-1X2C (S) bin/docket show names the reader's own branch as a carrier and stays silent about the other branch editing the same item
- PL-245B (S) docs/WORKING_NOTES.md cites test_vcs.py:467 as the test asserting _superseded's inverted direction, but that line has never held such a test - PL-Q9Z1's live thread rests on it
...
- PL-DMNX (S) PL-1DN9's premise that nothing repository-side runs when a pull request is created is now false, so the published-body read-back it bought 337 resident characters for may be scriptable
- PL-FT3M (S) cli._cuts's docstring says the unlanded-ref walk is gated on the release offer, where it is gated on Readiness.is_worth_cutting - and the two now diverge routinely
- PL-G8TR (S) no-prune-guard is evaded by the form it recommends - git branch -dr driven from a generated list is a prune
...
- PL-JTHW (S) tools/main_ci_status.py passes over a cancelled main run in silence, so after PL-SMN4 a commit with no whole-store verdict is indistinguishable from one nobody asked about
- PL-JW39 (S) docket next ranks a needs-decision item first, so every fresh session opens on work whose next step is the owner's answer
- PL-JW9J (S) core_vocabulary_check matches whole identifiers because two live names would have failed a substring rule, and since PL-6KNM neither exists: decide whether to tighten
- PL-KFWL (S) The v0.4.8 tag is pushed onto a commit where the release was never cut, so doc_check errors on main for every session
- PL-KNHX (S) contrast_check's KNOWN_SHORTFALLS entries are never checked against the requirements they excuse, so a stale one lingers and inflates the reported shortfall count
- PL-KRS6 (S) bin/docket release counts the previous release's own cut item as releasable work, so every session after a release is offered an empty one
- PL-LBW5 (S) Five open items' verify: commands grep a file their own touches does not declare, so bin/docket concurrent gives a wrong answer for work that will certainly edit it
- PL-LF2C (S) PL-VV4D's exact left-behind check rests on refs/pull/<n>/head being permanent, and GitHub is about to unreference 90 of them, so the check needs a third decline condition and one of its two test vectors dies
...
- PL-P4XB (S) test_verify.py's _commission helper commits unconditionally, so calling it with fields identical to the base store's copy dies inside subprocess with CalledProcessError rather than saying nothing changed
- PL-PFK1 (S) bin/docket verify REJECTs a self-audited branch on commission checks alone without ever naming --self, so a session reaching the command from CLAUDE.md or --help still hits the wall PL-7XTS closed for the skill
- PL-PNW6 (S) A release cut at a version number some withdrawn tag once named leaves every warm checkout pointing v<version> at the old commit, and the handover's own 'git fetch origin main' is the command that leaves it stale silently
- PL-QNYF (S) docket check errors on an item whose closure landed in a queue-only commit, and the error names no remedy the session can reach
- PL-QSGX (S) bin/docket flight never fetches, unlike stranded and branch, so it reports refs as old as the clone - the one command whose whole job is reading other sessions' branches
- PL-QV5Y (S) Makefile's CI-timing comments quote a 1230-test suite at 78 s serial and 27 s parallel, measured 2026-09-03; the suite is now 2096 tests at about 43 s parallel, so a reader sizing a CI-cost decision from them is reading stale figures
- PL-RWBV (S) Seven open items declare touches paths that do not exist, three of them naming core/run_score.py which PL-ZX12 renamed to run_definition.py
- PL-T441 (S) The git reads behind a printed count resolve the store from config.items_dir rather than the tracked directory, so docket check --items pointed at a store elsewhere silently finds no closures
- PL-T8PT (S) make check runs pr_title_check --discover against committed history, so a session that runs it before committing the closure sees a HEAD without it, passes locally, and goes red in CI anyway
- PL-TP5M (S) PL-ZBRB's advisory names blocked-by to work around a checker that did not read it, and PL-KBD0's check now does
- PL-VJFQ (S) Nothing enforces that KNOWN_SHORTFALLS only shrinks, so the contrast ledger could become the suppression list ui-color.md forbids in prose
- PL-VJPJ (S) bin/docket next raises BrokenPipeError and prints a traceback when its output is piped into a command that closes early, so next | head looks like a crash to every session that pipes it
- PL-VV6N (S) Decide a retention rule for items captured but never worked
- PL-VYK1 (S) The docket skill's release handover tags at origin/main rather than at the cut's own merge commit, so a re-run of the three commands tags whatever merged next
- PL-WFFX (S) Name the merge client that sends an empty commit_message, which only the project owner can do: affected merges cluster by sitting at 98.3% and by owner-local hour at p=5e-5, but GitHub records no client identity
- PL-WG7Q (S) _merges_naming's subject scan applies only the closed-here test and never the carried-work one, so a closure split from its work is recorded whenever its own subject leads with the id
- PL-WHQS (S) Centralize the closed-item filter now duplicated across twelve call sites
- PL-WTXB (S) Remove the unreferenced docket milestone command
- PL-WVJ0 (S) The docket skill says bin/docket show prints no placement at all, but PL-J790 added the plan: line to show in v0.4.18, so a session is sent to look up what the command already prints
...
24. Add a user-facing preferences/settings panel (theme, chart window, slider
    ranges, and similar display settings).

    *The consolidation this item named as its prerequisite has landed, and what
    replaces it is a different shape of work* (`PL-YD6X`). The prerequisite
    read: fold the UI/display constants then scattered across `app/theme.py`,
    `app/simulation_view.py`'s module-level constants and the defaults
    duplicated between `core/*.py` dataclasses and `app/controller.py` into one
    module, rather than adding a fourth scattered location. `PL-2CS8` did that
    in `v0.4.11`: `app/theme.py` is the one location, `app/controller.py`
    declares no module-level constant and no `core/` default of its own, and
    what `app/simulation_view.py` still declares is Qt layout policy — stretch
    factors and object names — rather than anything a reader would set.

    What genuinely remains is not a fourth location to fold in. `app/theme.py`
    is a module of constants bound at import, so a panel that *writes* a
    setting needs something that can be written and persisted, and this
    repository has no such store yet. That is the prerequisite to scope, and it
    is a build rather than a consolidation.
    ...
    *What the panel must never make editable, and the test that decides it*
    ...
    `CLAUDE.md`'s "the correct number with the wrong units, label" exactly. The last
...
26. Add run bookmarks that halt a run at a target, after item 25. There is
    ...
    *Required properties.* Crossings are tested on every simulation step,
    ...
    taken from that bookmark. *Crossing direction was part of this floor and is
    withdrawn* (project owner, 2026-09-20): a height is a height, and a run
    that passes through one on the way up and again on the way down has
    reached what the learner marked both times, so a target names a
    compartment and a height and nothing else — held as a multiple of 1 MAC
...
33. Run the interface pass: one deliberate visual design pass over the whole
    ...
    *The structural half is not part of this item, and it has landed.* Most of
    what an overhaul looks like from outside is not polish, and all three
    pieces of it are now closed. `PL-2CS8` consolidated the display constants
    item 24 named as its own prerequisite and "Development pathway" Phase 2
    orders first, in `v0.4.11`. `PL-B9PY` is the decomposition, which is also
    the component seam that makes the six repeated panel recipes one, in
    `v0.4.25`. `PL-NGF7` settled the theme-object question in `v0.4.26` — not
    by the Qt port dissolving it, which is what the deferral predicted and what
    the item then measured and found false, but by scoping what the stylesheet
    owns and adding the check that fails the build the moment a module under
    `app/` writes a colour outside that scope. None of the three is debt any
    gate still has to clear. What is left for this item is the part that is
    genuinely taste.
